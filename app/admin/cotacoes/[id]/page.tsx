...
export default function EditCotacaoPage() {
  ...
  const loadCotacao = async () => {
    try {
      ...
      const cotacao = data.cotacoes.find((c: any) => c.id === parseInt(id))
      
      if (cotacao) {
        setFormData({
          name: cotacao.name || '',
          value: cotacao.value || '',
          modalidadeId: cotacao.modalidadeId?.toString() || '',
          extracaoId: cotacao.extracaoId?.toString() || '',
          promocaoId: cotacao.promocaoId?.toString() || '',
          isSpecial: cotacao.isSpecial || false,
          active: cotacao.active !== undefined ? cotacao.active : true,
        })
      }
    } catch (error) {
